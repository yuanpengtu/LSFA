...
DATASETS_PATH = 'datasets/mvtec3d_preprocessed'
RGB_SIZE = 224
...
class MVTec3DTrain(MVTec3D):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'img': img,
            'resized_organized_pc': resized_organized_pc,
            'label': label,
            'rgb_path': rgb_path[len(DATASETS_PATH):]
        }
...
class MVTec3DTest(MVTec3D):
    ...
    def __getitem__(self, idx):
        ...
        if gt == 0:
            gt = torch.zeros(
                [1, resized_organized_pc.size()[-2], resized_organized_pc.size()[-2]])
        else:
...
def get_data_loader(split, class_name, img_size):
    ...
    data_loader = DataLoader(dataset=dataset, batch_size=1, shuffle=False, num_workers=0, drop_last=False,
                             pin_memory=True)
    return data_loader
