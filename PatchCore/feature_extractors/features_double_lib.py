...
class Features(torch.nn.Module):
    ...
    def __init__(self, args, image_size=224, f_coreset=0.1, coreset_eps=0.9):
        ...
        self.image_preds_ori = list()
        self.predictions_ori = []        
        self.pixel_preds_ori = list()


        self.fusion_block = FeatureFusionBlock()
        """
        if args.epoch == 0:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90_rgb900_finetuneblock.pkl'), strict=False)
            print("rgb point all rotate 90 0fusion_block:", msg)
        elif args.epoch ==1:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90_rgb901_finetuneblock.pkl'), strict=False)
            print("rgb point all rotate 90 1fusion_block:", msg)
        elif args.epoch == 2:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90180_rgb901800_finetuneblock.pkl'), strict=False)
            print("rgb point all rotate 90180 0fusion_block:", msg)
        elif args.epoch == 3:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90180_rgb901801_finetuneblock.pkl'), strict=False)
            print("rgb point all rotate 90180 1fusion_block:", msg)
        elif args.epoch == 4:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90180270_rgb901802700_finetuneblock.pkl'), strict=False)
            print("rgb point all rotate 90180270 0fusion_block:", msg)
        elif args.epoch == 5:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90180270_rgb901802701_finetuneblock.pkl'), strict=False)
            print("rgb point all rotate 90180270 1fusion_block:", msg)
        elif args.epoch == 6:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90_rgb901800_finetuneblock.pkl'), strict=False)
            print("rgb90180 point90 all rotate 0fusion_block:", msg)
        elif args.epoch == 7:
            msg = self.fusion_block.load_state_dict(torch.load('/youtu_fuxi_team1_ceph/yuanpengtu/noise_3D/M3DM_pretrainfeatv2/savemodel/wangblocksALLepoch_pointrotateall90_rgb901801_finetuneblock.pkl'), strict=False)
            print("rgb90180 point90 all rotate 1fusion_block:", msg)
        """
        ...
        self.detect_fuser_ori = linear_model.SGDOneClassSVM(random_state=42)
        self.seg_fuser_ori = linear_model.SGDOneClassSVM(random_state=42)
    ...
    def compute_s_s_map(self, xyz_patch, rgb_patch, feature_map_dims, mask, label, center, neighbour_idx,
        ...
        s_ori = torch.tensor(self.detect_fuser_ori.score_samples(s))
        s_map_ori = torch.tensor(self.seg_fuser_ori.score_samples(s_map))
        s_map_ori = s_map_ori.view(1, self.image_size, self.image_size)
        ...
        self.image_preds_ori.append(s_ori.numpy())
        self.pixel_preds_ori.extend(s_map_ori.flatten().numpy())
        self.predictions_ori.append(s_map_ori.detach().cpu().squeeze().numpy())
    ...
    def calculate_metrics(self):
        ...
        self.image_preds_ori = np.stack(self.image_preds_ori)
        self.pixel_preds_ori = np.array(self.pixel_preds_ori)

        self.image_rocauc_ori = roc_auc_score(self.image_labels, self.image_preds_ori)
        self.pixel_rocauc_ori = roc_auc_score(self.pixel_labels, self.pixel_preds_ori)
        self.au_pro_ori, _ = calculate_au_pro(self.gts, self.predictions_ori)
    ...
    def run_late_fusion(self):
        ...
        self.s_lib_ori = torch.cat(self.s_lib_ori, 0)
        self.s_map_lib_ori = torch.cat(self.s_map_lib_ori, 0)
        self.detect_fuser_ori.fit(self.s_lib_ori)
        self.seg_fuser_ori.fit(self.s_map_lib_ori)
