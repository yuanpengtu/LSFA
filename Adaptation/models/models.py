...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, pos, flag=False):
        feature_list = []
        fetch_idx = [3, 7, 11]
...
class PointTransformer(nn.Module):
    ...
    def forward(self, neighborhood,  center, ori_idx, center_idx):
        if self.encoder_dims != self.trans_dim:
            #B,C,N = pts.shape
            ...
            feature_list = torch.cat((feature_list[0],feature_list[1],feature_list[2]), dim=1) #1152

            return feature_list, center, ori_idx, center_idx
